# coding: utf-8
"""
Traffic volume and host-orientation analysis helpers.
...
def time_flow(PCAPS):
    """Return packet sizes keyed by both first timestamp and relative capture time."""
    time_flow_dict = collections.OrderedDict()
    start = PCAPS[0].time
    time_flow_dict[time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(float(PCAPS[0].time)))] = len(corrupt_bytes(PCAPS[0]))
    for pcap in PCAPS:
...
def get_host_ip(PCAPS):
    """Infer the local host by picking the most frequently seen IPv4 address."""
    ip_list = list()
    for pcap in PCAPS:
        if pcap.haslayer(IP):
...
def data_flow(PCAPS, host_ip):
    """Count packets moving into and out of the inferred host."""
...
def data_in_out_ip(PCAPS, host_ip):
    """Summarize peer packet counts and byte totals for inbound and outbound traffic."""
...
def proto_flow(PCAPS):
    """Accumulate total bytes per broad protocol family for dashboard charts."""
...
def most_flow_statistic(PCAPS, PD):
    """Return total bytes grouped by the decoded application or transport label."""
