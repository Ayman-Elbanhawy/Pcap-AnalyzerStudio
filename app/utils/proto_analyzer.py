# coding: utf-8
"""
Protocol-count statistics used by the analysis dashboards.
...
def pcap_len_statistic(PCAPS):
    """Bucket packets into the size ranges used by the dashboard pie chart."""
...
def common_proto_statistic(PCAPS):
    """Count packets by the broad protocol families shown in the summary charts."""
...
def most_proto_statistic(PCAPS, PD):
    """Return the ten most common decoded protocol labels in the capture."""
    protos_list = list()
    for pcap in PCAPS:
...
def http_statistic(PCAPS):
    """Count HTTP and HTTPS conversations by the remote IP endpoint."""
    http_dict = dict()
    for pcap in PCAPS:
        if pcap.haslayer(TCP):
            tcp = pcap.getlayer(TCP)
...
def dns_statistic(PCAPS):
    """Count requested DNS query names for the DNS statistics chart."""
    dns_dict = dict()
    for pcap in PCAPS:
