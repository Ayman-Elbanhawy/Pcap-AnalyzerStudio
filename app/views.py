# coding: utf-8
"""
Flask routes for the Pcap Analyzer Studio web interface.
...
# Import helper functions into templates
app.jinja_env.globals['enumerate'] = enumerate
app.jinja_env.globals["github_repository"] = app.config.get("GITHUB_REPOSITORY", "")

# The current codebase still stores the active capture in module globals rather than
# per-user session state. That is sufficient for the current single-user desktop flow.
PCAP_NAME = ''  # uploaded file name
PD = PcapDecode()  # parser
PCAPS = None  # packets


def require_loaded_capture():
    """Redirect to upload when analysis routes are opened before loading a capture."""
    if PCAPS is None:
        flash("Please upload a packet capture before running analysis.")
...
def index():
    """Render the main shell page that hosts the sidebar and content iframe."""
    return render_template('./home/index.html')


@app.route('/upload/', methods=['POST', 'GET'])
def upload():
    """Accept a user-supplied capture file, store it, and parse it with Scapy."""
    ...
    if request.method == 'GET':
        ...
        if upload.validate_on_submit():
            ...
            if allowed_file(pcapname):
                ...
                flash('Upload failed. Please upload an allowed packet capture format.')
                return render_template('./upload/upload.html')
        else:
            return render_template('./upload/upload.html')
...
def basedata():
    """Render the packet table, optionally filtered by protocol or endpoint."""
    global PCAPS, PD
    missing_capture = require_loaded_capture()
    if missing_capture:
...
def datashow():
    """Return the selected packet details as HTML and stage a PDF export copy."""
    missing_capture = require_loaded_capture()
    if missing_capture:
        return missing_capture
...
def savepdf():
    """Download the PDF file generated by the most recent packet-detail request."""
    missing_capture = require_loaded_capture()
    if missing_capture:
        return missing_capture
...
def protoanalyzer():
    """Render protocol-level summary charts for the loaded capture."""
    missing_capture = require_loaded_capture()
    if missing_capture:
        return missing_capture
    ...
    http_dict = sorted(http_statistic(PCAPS).items(), key=lambda d: d[1], reverse=False)
    http_key_list = list()
    http_value_list = list()
    for key, value in http_dict:
        http_key_list.append(key)
        http_value_list.append(value)
    dns_dict = sorted(dns_statistic(PCAPS).items(), key=lambda d: d[1], reverse=False)
    dns_key_list = list()
    dns_value_list = list()
...
def flowanalyzer():
    """Render byte-volume and peer-traffic charts for the loaded capture."""
    missing_capture = require_loaded_capture()
    if missing_capture:
        return missing_capture
    ...
    most_flow_dict = sorted(most_flow_statistic(PCAPS, PD).items(), key=lambda d: d[1], reverse=True)
...
def ipmap():
    """Render the world map view for remote peers seen in the capture."""
    missing_capture = require_loaded_capture()
    if missing_capture:
        return missing_capture
...
def help_page():
    """Render the in-application help center and GitHub onboarding guide."""
